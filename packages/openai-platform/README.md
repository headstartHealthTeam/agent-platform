...
Its implementation lives in `local-operator-runtime.ts`; the environment-neutral adapter in
`operator-runtime.ts` depends only on the `SessionExecutor` port, not the Docker implementation or
workstation credential resolver. Optional local compute does not define the hosted runtime contract.
